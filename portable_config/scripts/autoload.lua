...
local function table.filter(t, f, iter)
    for i=#t, 1, -1 do
        if not iter(t[i], f) then table.remove(t, i) end
    end
end

local function filterfunction(v, f)
...
local function Load(dir, filename, files)
    local current
    for i = 1, #files do
...
local function main()
    ...
    mp.msg.info("Loading "..#files.." files.")
